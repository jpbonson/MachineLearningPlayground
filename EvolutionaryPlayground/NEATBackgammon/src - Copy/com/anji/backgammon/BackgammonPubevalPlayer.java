...
public class BackgammonPubevalPlayer implements BackgammonPlayer, Configurable
{
    private static final String playerId = "Backgammon Pubeval Player";
    ...
    public BoardState move( ArrayList<BoardState> moves ) {
        ...
        else
        {
            ...
            {
                gammonInput = board.codifyToNEAT_pubeval();
                output = pubeval(board.checkForRacingSituation(), gammonInput);
                moveList[i] = output;
                i++;
            }
    ...
    public double pubeval(boolean race, int pos[]) {
		   ...
		   * of checkers at each location), and W is a weight vector.  Separate weight
		   ...
		   * locations 1-24 from computer's point of view, i.e. computer's checkers move in
		   * the negative direction from 24 to 1, and opponent's checkers move in the
		   * positive direction from 1 to 24. Computer's checkers are represented by
		   * positive integers, and opponent's checkers are represented by negative
		   * integers. Element 25 represents computer's checkers on the bar (positive
		   * integer), and element 0 represents opponent's checkers on the bar (negative
		   * integer). Element 26 represents computer's checkers off the board (positive
		   * integer), and element 27 represents opponent's checkers off the board
		   * (negative integer).      */
		  ...
		  if (pos[26] == 15)
			    return (99999999.);
		  /* all checkers off, best possible move */
		...
		for (j = 1; j <= 24; ++j) {
			...
			if (n != 0) {
				...
				if (n == 3)
					x[5 * jm1 + 3] = 1.0;
				if (n >= 4)
					x[5 * jm1 + 4] = (double) (n - 3) / 2.0;
			}
		}
		/* encode opponent bar-checkers */
		x[120] = -(double) (pos[0]) / 2.0;
		/* encode computer's checkers-off */
		x[121] = (double) (pos[26]) / 15.0;
	}
